...
function IR_shortcode( $atts ) 
{
	...
	$arr = array();
	$arr["IR_TextLength"] = $atts['length'];
	$arr["IR_SameTime"] = $atts['display'];
	$arr["IR_Height"] = $atts['height'];
	$arr["IR_type"] = $atts['group'];
	$arr["IR_random"] = $atts['random'];
	$arr["IR_speed"] = $atts['speed'];
	$arr["IR_waitseconds"] = $atts['wait'];
	return IR_Show_Widget($arr);
}
...
function IR_Show_Widget( $atts ) 
{
	...
	if($IR_type <> "" )
	{
		$sSql = $sSql . " and IR_type = %s ";
		$sSql = $wpdb->prepare($sSql, $IR_type);
	}
	if($IR_random == "YES"){ $sSql = $sSql . " ORDER BY RAND()"; }else{ $sSql = $sSql . " ORDER BY IR_order"; }
	$IR_data = $wpdb->get_results($sSql);
	
	
    $IRFull .= '<style type="text/css">';
	$IRFull .= '.IR-regimage img {';
		$IRFull .= 'float: left;';
		$IRFull .= 'vertical-align:bottom;';
		$IRFull .= 'padding: 3px;';
	$IRFull .= '}';
	$IRFull .= '</style>';
	...
	if ( ! empty($IR_data) ) 
	{
		...
		$IRFull .= '<div style="padding-top:8px;padding-bottom:8px;">';
		 $IRFull .= '<div style="text-align:left;vertical-align:middle;text-decoration: none;overflow: hidden; position: relative; margin-left: 3px; height: ' . $IR_Height . 'px;" id="IRHolder">';
		 	$IRFull .= $IRhtml;
		 $IRFull .= '</div>';
		$IRFull .= '</div>';
		$IRFull .= '<script type="text/javascript">';
		$IRFull .= 'var IR	= new Array();';
		$IRFull .= "var objIR	= '';";
		$IRFull .= "var IR_scrollPos 	= '';";
		$IRFull .= "var IR_numScrolls	= '';";
		$IRFull .= "var IR_heightOfElm 	= '".$IR_Height."';";
		$IRFull .= "var IR_numberOfElm 	= '".$IR_count."';";
		$IRFull .= "var IR_speed 		= '".$IR_speed."';";
		$IRFull .= "var IR_waitseconds 	= '".$IR_waitseconds."';";
		$IRFull .= "var IR_scrollOn 	= 'true';";
		$IRFull .= 'function createIRScroll() ';
		$IRFull .= '{';
			$IRFull .= $IR_x;
			$IRFull .= "objIR	= document.getElementById('IRHolder');";
			$IRFull .= "objIR.style.height = (IR_numberOfElm * IR_heightOfElm) + 'px';";
			$IRFull .= "IRContent();";
		$IRFull .= '}';
		$IRFull .= '</script>';
		$IRFull .= '<script type="text/javascript">';
		$IRFull .= 'createIRScroll();';
		$IRFull .= '</script>';	
	}
	else
	{
...
function IR_adminscripts() 
{
	if( !empty( $_GET['page'] ) ) 
	{
		switch ( $_GET['page'] ) 
		{
			case 'information-reel':
				wp_register_script( 'IR-adminscripts', plugins_url( 'pages/information-reel-setting.js', __FILE__ ), '', '', true );
				wp_enqueue_script( 'IR-adminscripts' );
				$IR_select_params = array(
					'IR_path'   	=> __( 'Please enter the image path.', 'IR-select', 'information-reel' ),
					'IR_link'  		=> __( 'Please enter the target link.', 'IR-select', 'information-reel' ),
					'IR_target'   	=> __( 'Please enter the target status.', 'IR-select', 'information-reel' ),
					'IR_title'  	=> __( 'Please enter the image title.', 'IR-select', 'information-reel' ),
					'IR_desc'   	=> __( 'Please enter the image description.', 'IR-select', 'information-reel' ),
					'IR_type'   	=> __( 'Please select the content group/type.', 'IR-select', 'information-reel' ),
					'IR_type1'   	=> __( 'Please enter the gallery type.', 'IR-select', 'information-reel' ),
					'IR_status' 	=> __( 'Please select the display status.', 'IR-select', 'information-reel' ),
					'IR_order' 		=> __( 'Please enter the display order, only number.', 'IR-select', 'information-reel' ),
					'IR_delete'		=> __( 'Do you want to delete this record?', 'IR-select', 'information-reel' ),
				);
				wp_localize_script( 'IR-adminscripts', 'IR_adminscripts', $IR_select_params );
				break;
		}
	}
}

add_shortcode( 'information-reel', 'IR_shortcode' );
...
add_action( 'admin_enqueue_scripts', 'IR_adminscripts' );
?>
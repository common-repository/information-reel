...
if (isset($_POST['IR_form_submit']) && $_POST['IR_form_submit'] == 'yes')
{
	...
	$form['IR_path'] = isset($_POST['IR_path']) ? esc_url_raw($_POST['IR_path']) : '';
	if ($form['IR_path'] == '')
	{
	...
	$form['IR_link'] = isset($_POST['IR_link']) ? esc_url_raw($_POST['IR_link']) : '';
	if ($form['IR_link'] == '')
	{
	...
	if($form['IR_target'] != "_blank" && $form['IR_target'] != "_parent" && $form['IR_target'] != "_self" && $form['IR_target'] != "_new")
	{
		$form['IR_target'] = "_blank";
	} 
		
	$form['IR_title'] = isset($_POST['IR_title']) ? wp_filter_post_kses($_POST['IR_title']) : '';
	
	$form['IR_desc'] = isset($_POST['IR_desc']) ? wp_filter_post_kses($_POST['IR_desc']) : '';
	
	$form['IR_order'] = isset($_POST['IR_order']) ? sanitize_text_field($_POST['IR_order']) : '';
	if(!is_numeric($form['IR_order'])) { $form['IR_order'] = 1; }
	
	$form['IR_status'] = isset($_POST['IR_status']) ? sanitize_text_field($_POST['IR_status']) : '';
	if($form['IR_status'] != "YES" && $form['IR_status'] != "NO")
	{
		$form['IR_status'] = "YES";
	}
	
	$form['IR_type'] = isset($_POST['IR_type']) ? sanitize_text_field($_POST['IR_type']) : '';
...
jQuery(document).ready(function($){
    $('#upload-btn').click(function(e) {
        ...
        .on('select', function(e){
            ...
            $('#IR_path').val(img_imageurl);
			$('#IR_title').val(img_imagetitle);
        });
    });
});
</script>
...
<div class="form-wrap">
	...
	<form name="IR_form" method="post" action="#" onsubmit="return IR_submit()"  >
      ...
      <input name="IR_path" type="text" id="IR_path" value="<?php echo $form['IR_path']; ?>" size="80" />
      ...
      <input name="IR_link" type="text" id="IR_link" value="<?php echo $form['IR_link']; ?>" size="80" />
      ...
      <input name="IR_title" type="text" id="IR_title" value="<?php echo esc_html(stripslashes($form['IR_title'])); ?>" size="80" />
      ...
      <input name="IR_desc" type="text" id="IR_desc" value="<?php echo esc_html(stripslashes($form['IR_desc'])); ?>" size="80" maxlength="1024" />
